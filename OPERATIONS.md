...
This is about keeping it running, not what it is — the code and
`AGENTS.md` are the source of truth for how the app itself works.
...
`ci.yml` checks every push/PR, `content-pipeline.yml` — the daily
pipeline — publishes that day's three readings and their author portraits
on its own once a day).
Vercel and `content-pipeline.yml` each need their own copy of
`DATABASE_URL`; `content-pipeline.yml` also needs `ANTHROPIC_API_KEY` and,
if you want it publishing real portraits, the `R2_*` secrets. The app itself is stateless besides the database
...
`scripts/add-daily.ts`), and fills in the three authors' portraits. There
is no catalogue, no rotation, and no review gate — the home page and
Archive are just reads of `daily_picks` (`lib/dailyPicks.ts`). The prompt
in the workflow file is the full spec; `seed/README.md` carries the
sourcing, rights, and quality rules it follows.

Operationally, what matters:

- **Every run is visible**: the Actions tab has the full log — what it
  discovered, what it published, what portraits it added. Nothing is
  silent.
- **The database is the record, not git**: `works`, `daily_picks`, and
  `authors.portrait_url` are all live writes. A normal run commits no
  files at all — `select * from daily_picks order by pick_date desc` (or
  the "Daily picks" card on `/account`) is how you see what ran.
- **To pause it**: disable the workflow from the Actions tab (or comment
  ...
  you fire it by hand while it's otherwise paused. A day the pipeline
  doesn't run is simply blank (`/api/daily-selection` returns a 503 "not
  published yet" and the Archive skips it) until you backfill it.
- **Running it by hand**: "Run workflow" in the Actions tab takes a `mode`
  — `full` (the daily default) or `portraits-only` (just the portrait
  step, for backfilling authors an earlier run missed). The `/account`
  "Fetch portraits" button dispatches `portraits-only`.
- **Backfilling or fixing a day**: run `add-daily` yourself for the date —
  ```bash
  npm run add-daily -- --date=2026-09-08 --category=poem --title="…" \
    --author="…" --year=1898 --source-name="Project Gutenberg" \
    --source-url="https://…" --text-file=/tmp/poem.txt --description="…" \
    --era=19th_century --difficulty=medium --tags=grief,nature
  ```
  It stages, promotes, and pins in one step, and re-pins (not duplicates)
  if that `(date, category)` slot is already filled.
- **To undo a bad pick**: archive the work —
  `update works set status = 'archived' where id = '…'` — which drops it
  from `works_feed`, so that day immediately reads as "not published" until
  you re-run `add-daily` for it. For a bad portrait:
  `update authors set portrait_url = null where name = '…'` (falls back to
  the initial mark immediately; the next run retries that author).
- **If it stops running or starts failing**: GitHub emails the repo's
  owner on workflow failure by default. Most common cause is a missing or
  expired secret — it now needs `DATABASE_URL` **and** `ANTHROPIC_API_KEY`
  (the latter both for the agent's auth and for `add-daily`'s description
  generation), plus the `R2_*` set for portraits; check
  **Settings → Secrets and variables → Actions**.

## Accounts & reviewers

The architecture, briefly: Auth.js (next-auth v5) with the Neon adapter
(`lib/auth.ts`, `lib/auth-db.ts`), Google as the only sign-in provider,
and a `users.role` of `reader` / `reviewer` / `admin`. Accounts are
entirely opt-in — anonymous reading, Archive, and Shuffle all work with
nothing configured; an account only adds reading history and the ability
to recommend works. `reviewer`/`admin` additionally unlock `/admin/review`
and the admin panel on `/account`. Operationally:
...
|---|---|---|
| `DATABASE_URL` | `.env.local` (local dev, gitignored) · Vercel project env vars · GitHub Actions repo secret | The app at runtime · `content-pipeline.yml` (the daily pipeline) · every `npm run` pipeline script |
| `ANTHROPIC_API_KEY` | GitHub Actions repo secret · optionally `.env.local` | `content-pipeline.yml`'s Claude Code agent step, and `scripts/add-daily.ts` / `lib/generateDescription.ts` for auto-writing a work's description (skipped, not an error, when absent) |
...
| `R2_ACCOUNT_ID` / `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` / `R2_BUCKET_NAME` / `R2_PUBLIC_URL` | `.env.local` · Vercel project env vars · GitHub Actions repo secrets (so the daily pipeline can publish real portraits) | Author-portrait storage — genuinely optional; without it every author just shows a generated initial instead of a real photo, nothing errors. **After setting these, run `npm run setup-r2-cors` once** — see Troubleshooting below for what happens if you skip it. |
| `GITHUB_DISPATCH_TOKEN` | `.env.local` · Vercel project env vars | The "Fetch portraits" button on `/account` (`app/api/admin/trigger-author-portraits`) — a fine-grained GitHub PAT scoped to this repo with Actions read/write, used only to call `workflow_dispatch` on `content-pipeline.yml` (in `portraits-only` mode). Optional — without it the button just returns a clear error; running the workflow by hand from the Actions tab still works either way. |
...
- **Moving the database**: create a new Neon (or any Postgres) project,
  apply the migrations in order, update `DATABASE_URL` everywhere (see the
  table above). There's no seed step — the catalogue is built up one day at
  a time by the pipeline; a fresh database just starts blank and fills in
  from the next run (or backfill dates by hand with `npm run add-daily`).
  The app has no other database-specific code — `lib/db.ts` is a single
  `neon(url)` call.
...
- **Moving the GitHub repo** (new org, new owner, a fork): update the
  ...
  `lib/githubDispatch.ts`'s `REPO` constant and the `USER_AGENT` strings in
  `lib/fetchCandidates.ts` / `scripts/*` are the only places the repo
  URL/slug is hardcoded — a quick grep for `PaulLin1/oneoneone` finds
  everything.
...
- **`next build` fails on Vercel but passes locally**: check the Vercel
  build log for the actual error first — every DB-touching route is
  server-rendered on demand, never prerendered, so `next build` never
  needs `DATABASE_URL` and a missing env var only ever shows up at
  runtime. A build-time failure is something else, most likely a
  dependency or Node-version mismatch. Vercel's Node version can be pinned in
